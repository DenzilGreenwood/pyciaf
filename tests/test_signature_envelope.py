...
from ciaf.watermarks.models import ArtifactEvidence, ArtifactType, WatermarkDescriptor, WatermarkType, ArtifactHashSet
import json
...
def test_signature_metadata_creation():
    ...
    assert metadata_restored.key_backend == KeyBackend.KMS  # String converted back to enum
...
def test_enum_serialization_consistency():
    ...
    for backend in backends:
        ...
        print(f"  ✅ {backend.value:15} -> '{metadata_dict['key_backend']}' -> {metadata_restored.key_backend.value}")
    ...
    for encoding in encodings:
        ...
        print(f"  ✅ {encoding.value:10} -> '{envelope_dict['signature_encoding']}' -> {envelope_restored.signature_encoding.value}")
