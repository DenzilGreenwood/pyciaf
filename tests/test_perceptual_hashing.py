...
try:
    ...
    print("❌ imagehash not available - install with: pip install imagehash")
    sys.exit(1)
...
def test_perceptual_hash_image_function():
    ...
    try:
        ...
        print(f"  ✅ Invalid algorithm correctly rejected")
...
def test_different_images_are_different():
    ...
    if distance == 0:
        print("  ℹ️  Images too similar (both patterned) - this is OK for perceptual hashing")
        print("     Perceptual hashes focus on structure, not exact content")
    else:
        assert not is_similar_image(hash1, hash2, threshold=5), \
