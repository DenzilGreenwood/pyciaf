...
pytestmark = pytest.mark.skipif(
    not PIL_AVAILABLE,
    reason="PIL (Pillow) required for integration workflow tests"
)
...
def test_end_to_end_text_watermarking():
    ...
    print("  ✅ Before/after hashes different (watermark applied)")
...
def test_image_perceptual_hashing_workflow():
    ...
    print("  ✅ Watermark applied (50x50 white square)")
    ...
    print("  ✅ Perceptual hashes show similarity despite watermark")
...
def test_signature_envelope_round_trip():
    ...
    assert (
        evidence_dict["signature"]["metadata"]["signing_service"] == "ciaf-test-signer"
    )
    ...
    print("  ✅ Signature envelope reconstructed")
    ...
    print("  ✅ Canonical dict correctly excludes signature")
...
def test_multi_artifact_verification():
    ...
    unrelated = (
        "Completely different content about weather patterns and climate change."
    )
    result_unrelated = verify_text_fragments(unrelated, fragments)

    assert result_unrelated.match_confidence < 0.5
    print(
        f"  ✅ Unrelated text verification: {result_unrelated.match_confidence:.2%} confidence (correctly low)"
    )
...
def test_complete_workflow_with_all_features():
    ...
    for i, frag in enumerate(fragments):
        ...
        assert len(frag.fragment_text) > 0
        print(
            f"  ✅ Fragment {i+1}: '{frag.fragment_text[:40]}...' ({len(frag.fragment_text)} chars)"
        )
    ...
    print("  ✅ All hash types present:")
    print("     Exact hashes: before/after")
    print("     Normalized hashes: before/after")
    print("     SimHash: before/after")
    ...
    print("     Contains: artifact, watermark, signature, hashes, simhash")
    ...
    if len(fragments) > 0:
        print(
            f"  ✅ Fragment verification: {result.match_confidence:.2%} confidence ({result.fragments_matched}/{len(fragments)} matched)"
        )
    else:
        print("  ⚠️  No fragments selected (text too short)")
    ...
    print("  ✅ Canonical hash matches signature payload")
    ...
    print("  ✅ Watermark preserved in distributed text")
