...
def test_fragment_text_field_population():
    ...
    hash_after = sha256_text(original_text + " [WATERMARK]")  # Simulated watermarked version
    ...
    assert len(fragments) > 0, f"Should select at least one fragment (got {len(fragments)})"
    ...
    for frag in fragments:
        ...
        assert frag.fragment_text in original_text, "fragment_text not found in original!"
...
def test_sliding_window_case_variation():
    ...
    if result is not None:
        pos, confidence = result
        print(f"  ✅ Found at position {pos} with confidence {confidence:.2f}")
        print(f"     (Case variation reduces confidence from exact match)")
    else:
        print(f"  ℹ️  No high-confidence match found (expected due to case sensitivity)")
        print(f"     Current implementation uses case-sensitive character matching")
...
def test_fragment_verification_workflow():
    ...
    assert result.fragments_matched == len(fragments), \
...
def test_hash_verification_after_match():
    ...
    assert "hash verified" in match_result.match_details.lower(), \
    ...
    if result2.fragments_matched > 0:
        ...
        if "hash mismatch" in match_result2.match_details.lower():
            assert match_result2.confidence < match_result.confidence, \
                "Confidence should be reduced for hash mismatch"
            print(f"  ✅ Confidence correctly reduced due to hash mismatch")
...
def test_bug_161_regression():
    ...
    assert len(fragment.fragment_text) != 64 or not all(c in '0123456789abcdef' for c in fragment.fragment_text.lower()), \
    ...
    assert len(fragment.fragment_hash_before) == 64, \
        "fragment_hash_before should be 64 chars"
    assert all(c in '0123456789abcdef' for c in fragment.fragment_hash_before.lower()), \
    ...
    print(f"  ✅ Fragment successfully verified in original text")
    print(f"  ✅ Confidence: {result.match_confidence:.2f}")
