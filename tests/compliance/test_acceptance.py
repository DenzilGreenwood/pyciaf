...
class TestEUAIActArticle14HumanOversight:
    ...
    def test_oversight_required_positive(self):
        ...
        print(f"✓ Oversight test passed - Receipt: {receipt.get_receipt_hash()[:16]}...")
    ...
    def test_oversight_required_negative(self):
        ...
        with pytest.raises(ComplianceError, match="Human oversight required but not completed"):
            self.framework.commit_inference(inference_meta)
        
        print("✓ Negative oversight test passed - Correctly blocked inference without oversight")
    ...
    def test_oversight_not_required(self):
        ...
        assert receipt is not None, "Receipt should be generated for non-oversight inference"
        print(f"✓ Non-oversight inference test passed - Receipt: {receipt.get_receipt_hash()[:16]}...")
...
class TestEUAIActArticle15Robustness:
    ...
    def test_robustness_required_positive(self):
        ...
        robustness_test = self.framework.compliance.robustness_manager.create_adversarial_test(
            epsilon=0.03,
            attack_method="pgd",
        ...
        security_proof = self.framework.compliance.robustness_manager.create_security_proof(
            security_property="integrity",
            proof_method="formal_verification",
            evidence={"verification_tool": "test_tool", "properties_verified": ["integrity"]}
        )
        ...
        print(f"✓ Robustness test passed - Receipt: {receipt.get_receipt_hash()[:16]}...")
    ...
    def test_robustness_required_negative(self):
        ...
        with pytest.raises(ValueError, match="EU AI Act Article 15 requires robustness testing"):
            self.framework.commit_model_checkpoint(model_meta)
        
        print("✓ Negative robustness test passed - Correctly blocked model without robustness testing")
...
class TestGDPRCompliance:
    ...
    def test_consent_required_positive(self):
        ...
        print(f"✓ GDPR consent test passed - Receipt: {receipt.get_receipt_hash()[:16]}...")
    ...
    def test_consent_required_negative(self):
        ...
        with pytest.raises(ValueError, match="GDPR consent required for personal data processing"):
            self.framework.commit_dataset_record(dataset_meta)
        
        print("✓ Negative GDPR test passed - Correctly blocked personal data without consent")
    ...
    def test_erasure_request(self):
        ...
        assert erasure_result["actor_id"] == "erasure_subject_001", "Erasure should be for correct actor"
        assert len(erasure_result["actions_taken"]) > 0, "Erasure actions should be taken"
        
        # Verify consent is withdrawn
        updated_consent = self.framework.compliance.gdpr_manager.consent_receipts[consent.consent_id]
        assert updated_consent.status.value == "withdrawn", "Consent should be withdrawn after erasure"
        
        print(f"✓ GDPR erasure test passed - Erasure ID: {erasure_result['erasure_id']}")
...
class TestNISTAIRMFMonitoring:
    ...
    def test_continuous_monitoring(self):
        ...
        alert_event = self.framework.compliance.monitoring_manager.create_drift_monitoring_event(
        ...
        print(f"✓ NIST monitoring test passed - Capsule: {monitoring_capsule['capsule_id']}")
...
class TestISO42001CorrectiveActions:
    ...
    def test_corrective_action_loop(self):
        ...
        tracked_actions = self.framework.compliance.corrective_action_manager.issue_tracking.get(issue_id, [])
...
class TestHIPAASOXAccessLogging:
    ...
    def test_capsule_verification_logging(self):
        ...
        access_summary = self.framework.compliance.access_control_manager.get_access_log_summary(days=1)
        
        assert access_summary["total_access_events"] > 0, "Should have access events"
        assert "capsule_verification" in access_summary["access_types"], "Should track capsule verification"
        
        print(f"✓ HIPAA/SOX access logging test passed - Events: {access_summary['total_access_events']}")
    ...
    def test_access_denied_logging(self):
        ...
        """
        # Log denied access
        denied_event = self.framework.compliance.access_control_manager.log_capsule_verification(
        ...
        print(f"✓ Access denial logging test passed - Denied count: {access_summary['denied_access_count']}")
...
class TestComplianceIntegration:
    ...
    def setup_method(self):
        ...
        self.framework = CIAFFramework("CIAF_Integration_Test", self.policy, self.signer)
    ...
    def test_end_to_end_compliance_workflow(self):
        ...
        checkpoint = self.framework.compliance.oversight_manager.create_oversight_checkpoint(
            decision_context={"patient_diagnosis": True},
            risk_level="high"
        )
        ...
        assert compliance_report["gdpr_summary"]["active_consents"] > 0, "Should have active consents"
        assert compliance_report["robustness_summary"]["total_tests"] > 0, "Should have robustness tests"
        assert compliance_report["oversight_summary"]["completed_checkpoints"] > 0, "Should have completed oversight"
        ...
        print(f"  Proof capsule hash: {proof_capsule['verification']['capsule_hash'][:16]}...")
