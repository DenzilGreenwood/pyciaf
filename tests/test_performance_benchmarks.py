...
def benchmark_text_watermarking():
    ...
    for size in sizes:
        ...
        benchmark = PerformanceBenchmark(
            f"Text Watermarking ({size} chars)",
            iterations=50
        )
...
def benchmark_fragment_operations():
    ...
    benchmark = PerformanceBenchmark(
        f"Fragment Verification ({len(fragments)} fragments)",
        iterations=100
    )
...
def benchmark_hashing_algorithms():
    ...
    if PIL_AVAILABLE:
        ...
        for algo in ["phash", "ahash", "dhash", "whash"]:
            benchmark = PerformanceBenchmark(f"Perceptual Hash ({algo})", iterations=100)
            stats = benchmark.run(lambda a=algo: perceptual_hash_image(img_bytes, algorithm=a))
...
def benchmark_signature_envelope():
    ...
    benchmark = PerformanceBenchmark("Envelope Serialization (to_dict)", iterations=1000)
    ...
    benchmark = PerformanceBenchmark("Envelope Deserialization (from_dict)", iterations=1000)
...
def benchmark_signature_overhead():
    ...
    benchmark = PerformanceBenchmark("Evidence Serialization (no signature)", iterations=1000)
    ...
    benchmark = PerformanceBenchmark("Evidence Serialization (with signature)", iterations=1000)
...
def generate_performance_report(all_results: Dict[str, Any]):
    ...
    for size in [500, 1000, 5000, 10000]:
        ...
        if key in all_results["watermarking"]:
            ...
            print(f"  {size:5} chars: {stats['mean_ms']:7.2f} ms  ({stats['throughput_ops_per_sec']:6.1f} ops/sec)")
    ...
    for op in ["fragment_selection", "fragment_verification"]:
        if op in all_results["fragments"]:
            ...
            print(f"  {op_name:25}: {stats['mean_ms']:7.2f} ms  ({stats['throughput_ops_per_sec']:6.1f} ops/sec)")
    ...
    for algo in ["sha256", "normalized_hash", "simhash"]:
        if algo in all_results["hashing"]:
            ...
            print(f"  {algo.upper():15}: {stats['mean_ms']:7.2f} ms  ({stats['throughput_ops_per_sec']:7.1f} ops/sec)")
    ...
    if PIL_AVAILABLE:
        ...
        for algo in ["phash", "ahash", "dhash", "whash"]:
            ...
            if key in all_results["hashing"]:
                ...
                print(f"    {algo:6}: {stats['mean_ms']:7.2f} ms  ({stats['throughput_ops_per_sec']:6.1f} ops/sec)")
    
    print("\n📊 SIGNATURE ENVELOPE")
    print("-" * 60)
    for op in ["envelope_creation", "envelope_serialization", "envelope_deserialization"]:
        if op in all_results["signature"]:
            stats = all_results["signature"][op]
            op_name = op.replace("envelope_", "").replace("_", " ").title()
            print(f"  {op_name:20}: {stats['mean_ms']:7.2f} ms  ({stats['throughput_ops_per_sec']:7.1f} ops/sec)")
    ...
    print(f"  Overhead:           {overhead['overhead_ms']:7.2f} ms ({overhead['overhead_pct']:5.1f}%)")
    
    print("\n📊 COMPLETE WORKFLOW")
    print("-" * 60)
    stats = all_results["workflow"]["complete_workflow"]
    print(f"  End-to-End (2000 chars): {stats['mean_ms']:7.2f} ms  ({stats['throughput_ops_per_sec']:6.1f} ops/sec)")
    ...
    if watermark_1k < 50:
        ...
        print(f"     ⚠️  Watermarking > 50ms (needs optimization: {watermark_1k:.2f} ms)")
    
    if workflow_2k < 100:
        print(f"     ✅ Complete workflow < 100ms (target met: {workflow_2k:.2f} ms)")
    else:
        print(f"     ⚠️  Complete workflow > 100ms (needs optimization: {workflow_2k:.2f} ms)")
    
    if sig_overhead < 20:
        print(f"     ✅ Signature overhead < 20% (target met: {sig_overhead:.1f}%)")
    else:
        print(f"     ⚠️  Signature overhead > 20% (consider optimization: {sig_overhead:.1f}%)")
