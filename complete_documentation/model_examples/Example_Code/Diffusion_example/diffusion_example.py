...
def assess_generation_bias(generated_samples, prompts=None):
    ...
    if isinstance(generated_samples, list) and len(generated_samples) > 0:
        ...
        sample_array = np.array([np.array(img) if hasattr(img, '__array__') else np.random.rand(64, 64, 3) 
                               for img in generated_samples[:10]])  # Limit for demo
        
        if sample_array.size > 0:
            diversity = np.var(sample_array.flatten())
            bias_assessment['diversity_score'] = min(diversity / 1000.0, 1.0)  # Normalize
        
        # Check for potential biases (simplified)
        bias_assessment['potential_biases'] = [
            'Low diversity detected' if bias_assessment['diversity_score'] < 0.3 else 'Diversity acceptable',
...
def main():
    ...
    print(f"   Content types: Circles, Gradients, Noise patterns")
    print(f"   Bias categories: Neutral, Nature, Abstract")
    ...
    training_capsules = framework.create_provenance_capsules(
        "art_generation_training",
        generation_data_metadata["data_items"]
    )
    ...
    authentic_diffusion = AuthenticityTrackingDiffusion(model, scheduler, diffusion_params)
    ...
    print(f" Diffusion wrapper created with authenticity tracking")
    ...
    print(f"   Potential Biases:")
    for bias in bias_assessment['potential_biases']:
        print(f"     - {bias}")
    
    # Overall ethical assessment
    ethical_score = (bias_assessment['fairness_score'] + bias_assessment['diversity_score']) / 2
    ...
    for i, case in enumerate(content_test_cases):
        ...
        try:
            ...
            print(f"   Authenticity Hash: {response.get('authenticity_hash', 'N/A')[:16]}...")
    ...
    for i, (sample, auth_hash) in enumerate(zip(generated_samples[:3], authenticity_hashes[:3])):
        print(f"\n   Sample {i+1}:")
        
        # Verify authenticity hash
        verification_params = {
            'content': sample.numpy().tolist() if hasattr(sample, 'numpy') else str(sample),
        ...
        print(f"     Content Format: Valid tensor" if hasattr(sample, 'shape') else "String representation")
        print(f"     Authenticity: {' Verified' if authenticity_verified else ' Invalid'}")
        print(f"     Tamper Detection: {' No tampering' if authenticity_verified else ' Potential tampering'}")
    ...
    print(f" Ethical Monitoring Report:")
    print(f"   Content Safety: {ethical_monitoring['content_safety']['harmful_content']}")
    print(f"   Bias Level: {ethical_monitoring['content_safety']['bias_level']:.3f}")
    print(f"   Diversity Maintained: {'' if ethical_monitoring['content_safety']['diversity_maintained'] else ''}")
    print(f"   Authenticity Tracking: {' Active' if ethical_monitoring['authenticity_tracking']['all_content_hashed'] else ' Inactive'}")
    print(f"   Bias Monitoring: {' Active' if ethical_monitoring['bias_monitoring']['continuous_monitoring'] == 'enabled' else ' Inactive'}")
    ...
    print(f" Content Generation Audit Trail:")
    ...
    print(f"   Content Generations: {audit_trail.get('inference_connections', {}).get('total_receipts', len(generation_receipts))}")
    ...
    for i, receipt in enumerate(generation_receipts):
        ...
        print(f"   Generation Receipt {i+1}: {' Valid' if verification['receipt_integrity'] else ' Invalid'}")
    
    # Content compliance summary
    content_compliance = {
        "ethical_standards": {
            "content_safety": ethical_monitoring['content_safety']['harmful_content'] == 'none_detected',
            "bias_mitigation": ethical_monitoring['bias_monitoring']['fairness_score'] > 0.5,
            "diversity_requirement": ethical_monitoring['content_safety']['diversity_maintained'],
    ...
    print(f"\n Content Compliance Summary:")
    print(f"   Ethical Standards: {' Compliant' if content_compliance['ethical_standards']['ethical_compliance'] == 'full' else ' Non-compliant'}")
    print(f"   Authenticity Assurance: {' Full' if content_compliance['authenticity_assurance']['cryptographic_hashing'] else ' Partial'}")
    print(f"   Content Accountability: {' Complete' if content_compliance['regulatory_readiness']['content_accountability'] else ' Incomplete'}")
    print(f"   Regulatory Readiness: {' Ready' if content_compliance['regulatory_readiness']['audit_trail_complete'] else ' Not ready'}")
