        ...
        if result.stdout:
            try:
                ...
                completion_found = any(
                    "IMPLEMENTATION_COMPLETE" in line for line in stdout_lines
                )
                key_features_found = any(
                    "Key" in line
                    and (
                        "Features Demonstrated" in line or "Generation Features" in line
                    )
                    for line in stdout_lines
                )
                many_checkmarks = (
                    len([line for line in stdout_lines if "" in line]) >= 10
                )

                output_success = (
                    completion_found
                    or key_features_found
                    or any("" in line and "Complete!" in line for line in stdout_lines)
                    or many_checkmarks
                )
            except Exception:
...
def print_summary(results: List[Dict[str, Any]]):
    ...
    print("\nOverall Statistics:")
    print(
        f"   Successful Executions: {successful_runs}/{len(results)} ({successful_runs/len(results)*100:.1f}%)"
    )
    print(
        f"   Completed Examples: {completed_runs}/{len(results)} ({completed_runs/len(results)*100:.1f}%)"
    )
    ...
    print("\nIndividual Example Results:")
    print(
        f"{'Example':<25} {'Status':<10} {'Time':<8} {'Features':<10} {'Compliance':<12}"
    )
    print("-" * 75)
    ...
    for result in results:
        ...
        print(
            f"{result['name']:<25} {status:<10} {time_str:<8} {features:<10} {compliance:<12}"
        )
        ...
        if result["success"]:
            ...
            if result["stderr"]:
                ...
                if len(result["stderr"]) > 500:
                    print(
                        f"   ... (error truncated, full length: {len(result['stderr'])} chars)"
                    )
            if result["stdout"]:
                ...
                if len(result["stdout"]) > 300:
                    print(
                        f"   ... (output truncated, full length: {len(result['stdout'])} chars)"
                    )
            if not result["stderr"] and not result["stdout"]:
                print(
                    "   Error: No output captured - possible encoding or subprocess issue"
                )
    ...
    if successful_runs == len(results):
        ...
        print(
            f"   {len(failed_examples)} examples failed: {', '.join(failed_examples)}"
        )
        print("   Check dependencies and error messages")
        print("   Refer to README.md for troubleshooting")

    print("\nFor detailed documentation, see: README.md")
    print("For CIAF package information, visit: https://github.com/your-repo/ciaf")
...
def main():
    ...
    print("=" * 60)
    print(
        "This script will execute all AI model examples and provide a comprehensive summary."
    )
