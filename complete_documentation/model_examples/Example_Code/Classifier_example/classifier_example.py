    ...
    class MockCIAFFramework:
        ...
        def get_complete_audit_trail(self, model_name):
            return {
                ...
                "inference_connections": {"total_receipts": 3},
            }
...
def main():
    ...
    print("   Target: loan approval (binary classification)")
    ...
    training_capsules = framework.create_provenance_capsules(
        "credit_scoring_training", training_data_metadata["data_items"]
    )
    ...
    print(" Classifier wrapper created with fairness monitoring")

    # Train the wrapper with the same data
    print(" Training CIAF wrapper...")
    try:
        wrapper_training_data = [
            {
                "content": {
                    "features": X_train.iloc[i].values.tolist(),
        ...
        print(
            f" CIAF wrapper trained successfully: {wrapper_snapshot.snapshot_id[:16]}..."
        )
    ...
    demographic_parity = min(male_approval_rate, female_approval_rate) / max(
        male_approval_rate, female_approval_rate
    )
    ...
    print(
        f"   Fairness threshold (≥0.8): {' PASS' if demographic_parity >= 0.8 else ' FAIL'}"
    )
    ...
    male_tpr = ((test_predictions[male_mask] == 1) & (y_test[male_mask] == 1)).sum() / (
        y_test[male_mask] == 1
    ).sum()
    female_tpr = (
        (test_predictions[female_mask] == 1) & (y_test[female_mask] == 1)
    ).sum() / (y_test[female_mask] == 1).sum()

    # False positive rates by group
    male_fpr = ((test_predictions[male_mask] == 1) & (y_test[male_mask] == 0)).sum() / (
        y_test[male_mask] == 0
    ).sum()
    female_fpr = (
        (test_predictions[female_mask] == 1) & (y_test[female_mask] == 0)
    ).sum() / (y_test[female_mask] == 0).sum()

    tpr_parity = (
        min(male_tpr, female_tpr) / max(male_tpr, female_tpr)
        if max(male_tpr, female_tpr) > 0
        else 1.0
    )
    fpr_parity = (
        min(male_fpr, female_fpr) / max(male_fpr, female_fpr)
        if max(male_fpr, female_fpr) > 0
        else 1.0
    )
    ...
    print(
        f"   Equalized odds: {' PASS' if min(tpr_parity, fpr_parity) >= 0.8 else ' FAIL'}"
    )
    ...
    for i, idx in enumerate(sample_indices):
        ...
        print(
            f"     Prediction: {'Approved' if prediction == 1 else 'Rejected'} (prob: {probability:.3f})"
        )
    ...
    for i, case in enumerate(test_cases):
        ...
        feature_dict = dict(
            zip(["age", "income", "credit_score", "loan_amount"], case["features"])
        )

        # Make prediction through CIAF wrapper
        try:
            response, receipt = wrapped_classifier.predict(
                query=case["features"], model_version="v1.0"  # Pass features directly
            )
    ...
    print(
        f"   Inference Receipts: {audit_trail['inference_connections']['total_receipts']}"
    )
    # Safe access to optional keys
    integrity_verified = audit_trail.get("verification", {}).get(
        "integrity_verified", "Not available"
    )
    ...
    for i, receipt in enumerate(inference_receipts):
        ...
        print(
            f"   Receipt {i+1}: {' Valid' if verification['receipt_integrity'] else ' Invalid'}"
        )
    ...
    print("\n Compliance Summary:")
    print(
        f"   Fairness Score: {compliance_summary['fairness_assessment']['overall_fairness']:.3f}"
    )
    print(
        f"   Explainability: {compliance_summary['explainability']['transparency_score']}"
    )
    print(
        f"   Performance: {compliance_summary['performance']['accuracy']:.3f} accuracy"
    )
    print(
        f"   Audit Readiness: {compliance_summary['audit_compliance']['trail_completeness']}"
    )
