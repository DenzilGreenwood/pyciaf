    ...
    def demo_human_oversight(self):
        ...
        if self.demo_mode == "full":
            try:
                ...
                checkpoint = (
                    self.compliance.oversight_manager.create_oversight_checkpoint(
                        decision_context={
                ...
                print("  Reviewer: dr_martinez_oncologist")
        ...
        if self.demo_mode == "full":
            try:
                ...
                withdrawal_success = self.compliance.gdpr_manager.withdraw_consent(
                    consent.consent_id, actor_id="patient_maria_garcia"
                )
    ...
    def demo_robustness_security(self):
        ...
        if self.demo_mode == "full":
            try:
                ...
                print(
                    f"  Adversarial accuracy: {adv_test.metrics['adversarial_accuracy']:.3f}"
                )
                ...
                security_proof = (
                    self.compliance.robustness_manager.create_security_proof(
    ...
    def demo_continuous_monitoring(self):
        ...
        if self.demo_mode == "full":
            try:
                ...
                drift_event = (
                    self.compliance.monitoring_manager.create_drift_monitoring_event(
                ...
                print(
                    f"  KL divergence: {drift_event.metrics['kl_divergence']:.3f} (threshold: 0.1)"
                )
                print(
                    f"  PSI score: {drift_event.metrics['psi_score']:.3f} (threshold: 0.2)"
                )
                print(
                    f"  Alerts: {len(drift_event.alerts)} ({'NONE' if not drift_event.has_alerts() else ', '.join(drift_event.alerts)})"
                )
                ...
                print(
                    f"  KL divergence: {alert_event.metrics['kl_divergence']:.3f} (ALERT: > 0.1)"
                )
                print(
                    f"  PSI score: {alert_event.metrics['psi_score']:.3f} (ALERT: > 0.2)"
                )
    ...
    def demo_corrective_actions(self):
        ...
        if self.demo_mode == "full":
            try:
                ...
                print(
                    f"  Target demographic parity: {bias_action.success_metrics['demographic_parity']}"
                )
                print(f"  Verification: {bias_action.verification_method}")
                print(
                    f"  Status: {'COMPLETED' if bias_action.is_completed() else 'IN PROGRESS'}"
                )
                ...
                print("  Final demographic parity: 0.967 (target: 0.95) ✅")
                print("  Final equalized odds: 0.943 (target: 0.92) ✅")
                print("  Final stat parity diff: 0.033 (target: <0.05) ✅")
                ...
                tracked_actions = (
                    self.compliance.corrective_action_manager.issue_tracking.get(
    ...
    def demo_access_control(self):
        ...
        if self.demo_mode == "full":
            try:
                ...
                access_event = (
                    self.compliance.access_control_manager.log_capsule_verification(
                ...
                print("  Reason: Insufficient privileges")
                print(f"  IP: {denied_event.ip_address}")

                # Generate access log summary
                access_summary = (
                    self.compliance.access_control_manager.get_access_log_summary(
                        days=1
                    )
                )
                ...
                print(
                    f"  Capsule verifications: {access_summary['access_types'].get('capsule_verification', 0)}"
                )

                if access_summary["most_accessed_resources"]:
                    print("  Most accessed resources:")
                    for resource, count in list(
                        access_summary["most_accessed_resources"].items()
                    )[:3]:
    ...
    def demo_compliance_report(self):
        ...
        if self.demo_mode == "full":
            try:
                ...
                print(
                    f"  Total checkpoints: {report['oversight_summary']['total_checkpoints']}"
                )
                print(
                    f"  Completed: {report['oversight_summary']['completed_checkpoints']}"
                )
                ...
                print(
                    f"  Erasure requests: {report['gdpr_summary']['erasure_requests']}"
                )
                ...
                print(
                    f"  Total actions: {report['corrective_actions_summary']['total_actions']}"
                )
                print(
                    f"  Completed: {report['corrective_actions_summary']['completed_actions']}"
                )
