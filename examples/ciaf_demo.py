...
class CIAFDemo:
    ...
    def run_complete_flow(self):
        ...
        sec_proof = self.framework.compliance.robustness_manager.create_security_proof(
            ...
            evidence={
                "tool": "z3_prover",
                "properties": ["model_integrity", "input_validation"],
            },
        )
        ...
        print("  Reviewer: dr_smith_radiologist_001")
        ...
        print(
            f"  Inclusion proof valid: {proof_capsule['proofs']['inclusion_proof_valid']}"
        )
        print(f"  Signature valid: {proof_capsule['verification']['signature_valid']}")
        print(
            f"  Capsule hash: {proof_capsule['verification']['capsule_hash'][:16]}...\n"
        )
        ...
        if drift_event.has_alerts():
            ...
            print("  Final KL divergence: 0.04")
            print("  Final PSI score: 0.08")
        ...
        print(
            f"  Oversight checkpoints: {report['oversight_summary']['completed_checkpoints']}"
        )
        ...
        # Save artifacts
        self._save_demo_artifacts(
            dataset_receipt, model_receipt, inference_receipt, proof_capsule, report
        )
    ...
    def run_verification_demo(self, capsule_id: str):
        ...
        try:
            ...
            print(
                f"  Merkle path elements: {len(proof_capsule['proofs']['merkle_path'])}"
            )
            ...
            verification_result = self.framework.ledger.verify_merkle_path(
                leaf_hash, merkle_path, merkle_root
            )

            print("\n🔐 Independent Verification Results:")
            print(f"  Merkle path valid: {verification_result}")
            print(
                f"  Signature valid: {proof_capsule['verification']['signature_valid']}"
            )
            print(
                f"  Independently verifiable: {proof_capsule['verification']['verifiable_independently']}"
            )
    ...
    def run_erasure_demo(self, actor_id: str):
        ...
        erasure_result = self.framework.compliance.gdpr_manager.process_erasure_request(
            actor_id=actor_id, data_categories=["health", "biometric"]
        )

        print(f"✓ Erasure processed: {erasure_result['erasure_id']}")
        print(f"  Actor: {erasure_result['actor_id']}")
        print(
            f"  Categories: {', '.join(erasure_result['requested_categories'] or ['all'])}"
        )
        ...
        try:
            ...
            print("✓ Post-erasure verification access logged")
            print("✓ Cryptographic integrity preserved")
            print("  Merkle proofs remain valid (hashes preserved)")
            print("  Personal data erased, audit trail intact")
    ...
    def run_monitoring_demo(self, model_id: str):
        ...
        perf_event = (
            self.framework.compliance.monitoring_manager.create_drift_monitoring_event(
                kl_divergence=0.06, psi_score=0.12, model_version="2.0.0"
            )
        )
    ...
    def _save_demo_artifacts(
        self, dataset_receipt, model_receipt, inference_receipt, proof_capsule, report
    ):
...
def main():
    ...
    parser.add_argument(
        "--actor-id", default="patient_alice_001", help="Actor ID for erasure demo"
    )

    parser.add_argument(
        "--model-id", default="diagnostic_cnn_v2", help="Model ID for monitoring demo"
    )
