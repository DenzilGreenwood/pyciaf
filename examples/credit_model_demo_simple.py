...
def main():
    ...
    try:
        ...
        print(
            f"   🏛️  Compliance frameworks: {len(gdpr_wrapper.regulatory_frameworks)} ({', '.join(gdpr_wrapper.regulatory_frameworks)})"
        )
        ...
        print("   🔒 Privacy measures: Applied comprehensive GDPR protections")
        ...
        batch_queries = [
            f"Credit application: {json.dumps(app)}" for app in batch_applications
        ]
        ...
        for i, result in enumerate(batch_results):
            ...
            confidence = (
                receipt_info.get("confidence", 0.5)
        ...
        print(
            f"   🏛️  Frameworks validated: {len(compliance_report.get('regulatory_frameworks', []))}"
        )
        print(
            f"   🔒 Privacy measures: {len(compliance_report.get('privacy_measures', []))}"
        )
        print(
            f"   📊 Performance stats: {len(compliance_report.get('performance_stats', {}))}"
        )
        ...
        individual_receipt_data = {
            ...
            "privacy_measures": receipt_dict.get(
                "privacy_measures", ["GDPR_compliance_applied"]
            ),
            "query_data": test_app,
            "processing_details": {
