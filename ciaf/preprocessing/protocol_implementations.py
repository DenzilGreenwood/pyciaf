...
try:
    ...
    warnings.warn(
        "scikit-learn not available. Some preprocessing features will be limited."
    )
...
class DefaultTextPreprocessor(DataPreprocessor):
        ...
        try:
            ...
            if not SKLEARN_AVAILABLE:
                ...
                self.feature_names = sorted(list(unique_words))[
                    : self.policy.processing_policy.text_max_features
                ]
    ...
    def transform(
        self, data: Union[List[Dict[str, Any]], np.ndarray, str]
    ) -> np.ndarray:
        ...
        if not SKLEARN_AVAILABLE:
            ...
            for text in texts:
                if isinstance(text, str):
                    ...
                    features = [
                        1.0 if word in words else 0.0 for word in self.feature_names
                    ]
                else:
    ...
    def fit_transform(
        self, data: Union[List[Dict[str, Any]], np.ndarray]
    ) -> np.ndarray:
...
class DefaultNumericalPreprocessor(DataPreprocessor):
    ...
    def transform(
        ...
        if numerical_array.shape[1] != self.n_features:
            warnings.warn(
                f"Feature count mismatch: expected {self.n_features}, got {numerical_array.shape[1]}"
            )
            # Pad or truncate as needed
            if numerical_array.shape[1] < self.n_features:
                padding = np.zeros(
                    (
                        numerical_array.shape[0],
                        self.n_features - numerical_array.shape[1],
                    )
                )
...
class DefaultDataValidator(DataValidator):
    ...
    def generate_report(self, validation_results: Dict[str, Any]) -> str:
        ...
        # Overall status
        status = (
            "✅ PASSED" if validation_results.get("is_valid", False) else "❌ FAILED"
        )
    ...
    def _validate_content_quality(self, df: pd.DataFrame, results: Dict[str, Any]):
        ...
        for column in df.columns:
            ...
            if missing_ratio > max_missing_ratio:
                results["errors"].append(
                    f"Column '{column}' has too many missing values: {missing_ratio:.2%}"
                )
                results["is_valid"] = False
        ...
        results["metrics"]["avg_missing_ratio"] = (
            np.mean(list(missing_ratios.values())) if missing_ratios else 0
        )
                ...
                for column in numeric_columns:
                    ...
                    if len(col_data) > 0:
                        ...
                        if outlier_ratio > self.policy.quality_policy.outlier_threshold:
                            results["warnings"].append(
                                f"Column '{column}' has {outlier_ratio:.2%} outliers"
                            )
...
class DefaultModelAdapter(ModelAdapter):
    ...
    def fit(
        self, data: List[Dict[str, Any]], targets: Optional[List[Any]] = None
    ) -> bool:
        ...
        try:
            ...
            if self.preprocessor and self.preprocessor.fit(data):
                ...
                self.preprocessing_info = {
                    "data_type": (
                        data_type.value if "data_type" in locals() else "unknown"
                    ),
...
def create_default_preprocessing_protocols(
    policy: Optional[PreprocessingPolicy] = None,
) -> Dict[str, Any]:
...
def create_numerical_preprocessor(
    policy: Optional[PreprocessingPolicy] = None,
...
def create_auto_model_adapter(
    model: Any, policy: Optional[PreprocessingPolicy] = None
) -> DefaultModelAdapter:
