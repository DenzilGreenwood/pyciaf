...
def create_preprocessor(
    data_type: DataType, policy: Optional[PreprocessingPolicy] = None
) -> DataPreprocessor:
    """
...
def create_auto_preprocessor(
    data: Union[List[Dict[str, Any]], str], policy: Optional[PreprocessingPolicy] = None
) -> DataPreprocessor:
    """
...
def validate_data(
    data: Union[List[Dict[str, Any]], Any], policy: Optional[PreprocessingPolicy] = None
) -> Dict[str, Any]:
    """
...
class MixedDataPreprocessor:
    ...
    def __init__(
        ...
        policy.processing_policy.numerical_scaling = (
            "standard" if normalize_numerical else "none"
        )
...
def auto_preprocess_data(X, y=None, store_preprocessor=None):
    ...
    try:
        ...
        if isinstance(X[0], str):
            ...
            ciaf_format = [
                {"content": list(item) if hasattr(item, "__iter__") else [item]}
                for item in X
            ]
            X_processed = preprocessor.fit_transform(ciaf_format)
