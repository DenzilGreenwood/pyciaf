...
class DataQualityValidator:
    ...
    def validate(
        self, data: Union[List[Dict[str, Any]], pd.DataFrame, np.ndarray]
    ) -> ValidationResult:
        """
    ...
    def _validate_content(self, df: pd.DataFrame, result: ValidationResult):
        ...
        for column in df.columns:
            ...
            if missing_ratio > self.max_missing_ratio:
                result.add_error(
                    f"Column '{column}' has {missing_ratio:.2%} missing values (>{self.max_missing_ratio:.1%})"
                )
            ...
            if not col_data.isnull().all():
                ...
                if unique_count == 1:
                    ...
                    result.add_warning(
                        f"Column '{column}' has low diversity: {unique_ratio:.3%} unique values"
                    )

        result.add_metric("missing_value_ratios", missing_stats)
        result.add_metric(
            "overall_missing_ratio", np.mean(list(missing_stats.values()))
        )
    ...
    def _validate_statistical_quality(self, df: pd.DataFrame, result: ValidationResult):
        ...
        for column in numeric_columns:
            ...
            if outlier_ratio > 0.1:  # More than 10% outliers
                result.add_warning(
                    f"Column '{column}' has {outlier_ratio:.2%} potential outliers"
                )
    ...
    def _validate_duplicates(self, df: pd.DataFrame, result: ValidationResult):
        ...
        if duplicate_count > 0:
            if duplicate_ratio > 0.05:  # More than 5% duplicates
                result.add_error(
                    f"High duplicate ratio: {duplicate_ratio:.2%} ({duplicate_count} duplicates)"
                )
            else:
                result.add_warning(
                    f"Found {duplicate_count} duplicate records ({duplicate_ratio:.2%})"
                )
    ...
    def _validate_targets(self, df: pd.DataFrame, result: ValidationResult):
        ...
        target_columns = [
            col for col in df.columns if "target" in col.lower() or col == "y"
        ]
        ...
        for target_col in target_columns:
            ...
            if target_data.dtype in [np.number]:
                ...
                if class_balance < 0.1:  # Highly imbalanced
                    result.add_warning(
                        f"Target '{target_col}' is highly imbalanced (ratio: {class_balance:.3f})"
                    )
                    ...
                    if column in df.columns:
                        ...
                        compatible = (
                            (
                                ...
                                and np.issubdtype(actual_type, np.number)
                            )
                            or (expected_type is str and actual_type is object)
                            or str(actual_type) == str(expected_type)
                        )
...
def quick_validate(
    data: Union[List[Dict[str, Any]], pd.DataFrame, np.ndarray],
) -> ValidationResult:
...
def validate_ciaf_dataset(
    data: List[Dict[str, Any]], min_samples: int = 10, require_targets: bool = True
) -> ValidationResult:
    ...
    if require_targets:
        has_targets = any(
            "metadata" in item and "target" in item["metadata"] for item in data[:5]
        )
        if not has_targets:
            result.add_warning(
                "No target labels found in metadata - unsupervised learning assumed"
            )
