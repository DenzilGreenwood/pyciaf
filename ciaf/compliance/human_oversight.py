...
class OversightReview:
    ...
    def to_audit_record(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "intervention": (
                self.intervention_type.value if self.intervention_type else None
            ),
        }
...
class HumanOversightEngine:
    ...
    def __init__(
        self,
        model_name: str,
        oversight_level: OversightLevel = OversightLevel.REVIEW_ON_ALERT,
    ):
    ...
    def evaluate_inference_for_oversight(
        ...
        if prediction_confidence < self.oversight_thresholds["confidence_threshold"]:
            risk_factors.append(
                f"Low prediction confidence: {prediction_confidence:.3f}"
            )
            alert_needed = True
    ...
    def submit_review(
        ...
        alert_created = datetime.fromisoformat(
            alert.created_timestamp.replace("Z", "+00:00")
        )
        ...
        total_response_time = (
            self.metrics["average_response_time"] * (self.metrics["total_reviews"] - 1)
            + response_time
        )
        self.metrics["average_response_time"] = (
            total_response_time / self.metrics["total_reviews"]
        )
    ...
    def _handle_escalation(self, alert: OversightAlert, review: OversightReview):
        ...
        escalated_alert = OversightAlert(
            ...
            context={
                **alert.context,
                "escalated_from": alert.alert_id,
                "escalation_reason": review.rationale,
            },
            created_timestamp=datetime.now(timezone.utc).isoformat(),
    ...
    def get_oversight_metrics(self) -> Dict[str, Any]:
        ...
        escalations = len(
            [r for r in self.review_history.values() if r.escalation_needed]
        )
        interventions = len(
            [r for r in self.review_history.values() if r.intervention_type is not None]
        )
    ...
    def export_oversight_data(
        self, format: str = "json", include_context: bool = False
    ) -> str:
...
class OversightIntegration:
    ...
    def check_inference_oversight(
        self, model_name: str, inference_receipt, **kwargs
    ) -> Optional[OversightAlert]:
...
def demo_human_oversight():
    ...
    if alert_1:
        ...
        print(
            f"   Review submitted: {review_1.review_id} - Decision: {review_1.decision.value}"
        )
    ...
    if alert_2:
        ...
        print(
            f"   Review submitted: {review_2.review_id} - Decision: {review_2.decision.value}"
        )
    ...
    print(
        f"   Average response time: {metrics['avg_response_time_minutes']:.2f} minutes"
    )
