...
class DashboardData:
    ...
    def get_compliance_overview(self) -> Dict[str, Any]:
        ...
        try:
            metrics = {}
            if (
                hasattr(self.framework, "model_anchors")
                and self.framework.model_anchors
            ):
                for model_name in self.framework.model_anchors.keys():
                    try:
                        compliance_data = self.framework.get_compliance_status(
                            model_name
                        )
                        metrics[model_name] = compliance_data
                    except (AttributeError, KeyError):
                        # Framework doesn't have compliance status method
                        metrics[model_name] = self._generate_model_compliance_metrics(
                            model_name
                        )
        ...
        return {
            ...
            "frameworks": {
                ...
                "NIST_AI_RMF": {
                    ...
                    "status": "partial" if random.random() > 0.7 else "compliant",
                },
    ...
    def get_audit_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self.framework, "compliance") and hasattr(
                self.framework.compliance, "audit_trail"
            ):
        ...
        try:
            ...
            if hasattr(audit_trail, "events"):
                for event in audit_trail.events:
                    ...
                    if (
                        "violation" in event_type.lower()
                        or "breach" in event_type.lower()
                    ):
                        violation_count += 1
            ...
            if (
                ...
                for model_name, anchor in self.framework.model_anchors.items():
                    model_info = {
                        ...
                        "status": "active",
                        "compliance_score": self._calculate_model_compliance_score(
                            model_name, anchor
                        ),
    ...
    def _count_model_alerts(self, model_name: str) -> int:
        ...
        try:
            ...
            if hasattr(self.framework, "oversight_engine"):
                return len(
                    self.framework.oversight_engine.get_active_alerts(model_name)
                )
            else:
                return random.randint(0, 5)
        except Exception:
    ...
    def get_recent_alerts(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if hasattr(self.framework, "compliance") and hasattr(
                self.framework.compliance, "get_recent_violations"
            ):
                compliance_alerts = self.framework.compliance.get_recent_violations(
                    limit=5
                )
    ...
    def _generate_mock_alerts(self) -> List[Dict[str, Any]]:
        ...
        for i in range(num_alerts):
            ...
            descriptions = {
                ...
                "compliance_violation": "GDPR consent verification failed",
            ...
            alert = {
                ...
                "description": descriptions.get(
                    alert_type, f"Alert of type {alert_type}"
                ),
                "timestamp": (
    ...
    def _aggregate_compliance_metrics(self, model_metrics: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            "compliant_models": len(
                [m for m in model_metrics.values() if m.get("score", 0) >= 80]
            ),
            "frameworks": {},
            "recent_alerts": 3,
...
class CIAFDashboard:
    ...
    def __init__(self, ciaf_framework=None, host="127.0.0.1", port=5000, debug=False):
        if not FLASK_AVAILABLE:
            raise ImportError(
                "Flask is required for dashboard. Install with: pip install flask flask-socketio"
            )
...
<body>
    ...
    def run(self, **kwargs):
        ...
        self.socketio.run(
            self.app, host=self.host, port=self.port, debug=self.debug, **kwargs
        )
