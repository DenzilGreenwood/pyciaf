...
class BiasValidator:
    ...
    def validate_predictions(
        ...
        if metrics is None:
            ...
            if labels is not None:
                metrics.extend(
                    [BiasMetric.EQUALIZED_ODDS, BiasMetric.EQUAL_OPPORTUNITY]
                )
        ...
        fairness_scores = [
            r.metric_value for r in individual_results if r.metric_value is not None
        ]
        overall_fairness = np.mean(fairness_scores) if fairness_scores else 0.0

        # Generate summary statistics
        summary_stats = self._generate_summary_statistics(
            individual_results, protected_attributes
        )

        # Generate recommendations
        recommendations = self._generate_recommendations(
            individual_results, overall_fairness
        )

        # Determine compliance status
        compliance_status = self._assess_compliance_status(
            overall_fairness, individual_results
        )
        ...
        for group in unique_groups:
            ...
            try:
                if metric == BiasMetric.DEMOGRAPHIC_PARITY:
                    ...
                    metric == BiasMetric.EQUAL_OPPORTUNITY and group_labels is not None
                ):
                    metric_value = self._equal_opportunity(
                ...
                self.logger.warning(
                    f"Failed to compute {metric.value} for {attribute_name}={group}: {e}"
                )
    ...
    def _demographic_parity(self, predictions: np.ndarray) -> float:
        ...
        if predictions.dtype == float and np.all(
    ...
    def _calibration(self, predictions: np.ndarray, labels: np.ndarray) -> float:
        ...
        if not (
            ...
            and np.all((predictions >= 0) & (predictions <= 1))
        ):
            return 0.0
    ...
    def _generate_summary_statistics(
        self, results: List[BiasResult], protected_attributes: Dict[str, np.ndarray]
    ) -> Dict[str, Any]:
        ...
        if results:
            stats["fairness_scores"] = {
                "min": min(
                    r.metric_value for r in results if r.metric_value is not None
                ),
                "max": max(
                    r.metric_value for r in results if r.metric_value is not None
                ),
    ...
    def _generate_recommendations(
        self, results: List[BiasResult], overall_fairness: float
    ) -> List[str]:
    ...
    def _assess_compliance_status(
        self, overall_fairness: float, results: List[BiasResult]
    ) -> str:
