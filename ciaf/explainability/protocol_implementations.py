...
class ShapExplainer:
    ...
    def explain(self, X: np.ndarray, max_features: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            max_features = min(
                max_features, self.policy.quality_policy.max_features_in_explanation
            )
            ...
            explanation = {
                ...
                "feature_attributions": self._build_feature_attributions(
            ...
            if self.policy.explanation_level in [
    ...
    def _build_feature_attributions(
        self, shap_values: np.ndarray, top_features: List[Tuple[int, float]]
    ) -> List[Dict[str, Any]]:
    ...
    def _build_lime_attributions(self, lime_explanation) -> List[Dict[str, Any]]:
        ...
        for rank, (feature_idx, importance) in enumerate(lime_explanation.as_list()):
            feature_name = (
                ...
                and feature_idx < len(self.feature_names)
...
class DefaultExplanationValidator:
    ...
    def validate_feature_coverage(
        self, explanation: Dict[str, Any], min_features: int = 5
    ) -> bool:
    ...
    def validate_confidence_threshold(
        self, explanation: Dict[str, Any], min_confidence: float = 0.7
    ) -> bool:
        """Validate explanation confidence meets threshold."""
        confidence = explanation.get("explanation_confidence", 0.0)
        threshold = max(
            min_confidence, self.policy.quality_policy.min_explanation_confidence
        )
        return confidence >= threshold

    def validate_regulatory_compliance(
        self, explanation: Dict[str, Any], framework: str = "eu_ai_act"
...
def create_auto_explainer(
    ...
    for method in policy.method_preferences.preferred_methods:
        try:
            if method in [
                ...
                if method == ExplanationMethod.SHAP_TREE and hasattr(
                    ...
                    try:
                        ...
                        continue
                elif method == ExplanationMethod.SHAP_LINEAR and hasattr(
                    model, "coef_"
                ):
                    explainer = ShapExplainer(model, method, feature_names, policy)
                    try:
    ...
    if working_explainer is None:
        ...
        if policy.method_preferences.fallback_to_feature_importance and hasattr(
            model, "feature_importances_"
        ):
