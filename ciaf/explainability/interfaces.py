...
class ExplanationAuditor(Protocol):
    ...
    def audit_explainer_performance(
        self, model_id: str, test_data: np.ndarray, num_samples: int = 100
    ) -> Dict[str, Any]:
...
class FeatureAttributionProvider(Protocol):
    ...
    def get_global_feature_importance(
        self, model_id: str, importance_type: str = "mean_absolute"
    ) -> Dict[str, float]:
    ...
    def get_feature_interaction_analysis(
        self, model_id: str, X: np.ndarray, max_interactions: int = 10
    ) -> List[Dict[str, Any]]:
...
class ExplanationStorageProvider(Protocol):
    ...
    def store_explanation(
        self, model_id: str, prediction_id: str, explanation: Dict[str, Any]
    ) -> bool:
