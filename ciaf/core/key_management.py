...
class KeyBundle:
    ...
    def get_signer(self) -> Optional[Ed25519Signer]:
        ...
        if (
            self.metadata.key_type == KeyType.SIGNING
            and self.metadata.algorithm == SignatureAlgorithm.ED25519
            and self.private_key_pem
        ):
            return Ed25519Signer.from_private_key_pem(
                self.metadata.key_id, self.private_key_pem
            )
        return None
    ...
    def store_key(self, key_bundle: KeyBundle) -> bool:
        ...
        try:
            ...
            bundle_dict = {
                ...
                "key_material": (
                    key_bundle.key_material.hex() if key_bundle.key_material else None
                ),
            }
...
class KeyManager:
    ...
    def generate_signing_key(
        ...
        expires_at = (
            (now + timedelta(days=validity_days)).isoformat().replace("+00:00", "Z")
        )
        ...
        key_bundle = KeyBundle(
            metadata=metadata, private_key_pem=private_pem, public_key_pem=public_pem
        )
    ...
    def generate_master_key(
        ...
        # Create key bundle
        key_bundle = KeyBundle(metadata=metadata, key_material=key_material)
    ...
    def get_active_signing_key(self, purpose: str = "") -> Optional[KeyBundle]:
        ...
        signing_keys = self.key_store.list_keys(
            key_type=KeyType.SIGNING, status=KeyStatus.ACTIVE
        )
    ...
    def rotate_key(self, old_key_id: str, new_key_id: str) -> Optional[KeyBundle]:
        ...
        if old_key.metadata.key_type == KeyType.SIGNING:
            return self.generate_signing_key(
                new_key_id, old_key.metadata.purpose, tags=old_key.metadata.tags
            )
            ...
            return self.generate_master_key(
                new_key_id, old_key.metadata.purpose, key_length, old_key.metadata.tags
            )
    ...
    def cleanup_retired_keys(self, retention_days: int = 90) -> List[str]:
        ...
        for key_meta in retired_keys:
            if key_meta.retired_at:
                retired_date = datetime.fromisoformat(
                    key_meta.retired_at.replace("Z", "+00:00")
                )
...
def create_filesystem_key_manager(
    key_dir: str = "keys", key_validity_days: int = 365
) -> KeyManager:
...
def generate_ciaf_signing_key(
    key_id: str, purpose: str = "ciaf_signing"
) -> Ed25519Signer:
    """
...
def get_ciaf_signer(
    key_id: Optional[str] = None, purpose: str = "ciaf_signing"
) -> Optional[Ed25519Signer]:
    """
