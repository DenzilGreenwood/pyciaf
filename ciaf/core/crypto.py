...
from .constants import PBKDF2_ITERATIONS, KDF_DKLEN
...
def decrypt_aes_gcm(
    key: bytes, ciphertext: bytes, nonce: bytes, tag: bytes, aad: Optional[bytes] = None
) -> bytes:
    cipher = Cipher(
...
def derive_master_anchor(password: str, salt: bytes) -> bytes:
    ...
    return hashlib.pbkdf2_hmac(
        "sha256", password_bytes, salt, PBKDF2_ITERATIONS, dklen=KDF_DKLEN
    )
