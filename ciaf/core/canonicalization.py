...
def canonicalize_and_hash(
    metadata: Dict[str, Any], hash_algorithm: HashAlgorithm = HashAlgorithm.SHA256
) -> str:
    """
...
class WORMMerkleTree:
        ...
        while len(current_level) > 1:
            ...
            if current_index % 2 == 0:
                ...
                if sibling_index < len(current_level):
                    ...
                    proof.append(
                        (current_level[current_index], "right")
                    )  # Self if no right sibling
            else:
    ...
    def verify_merkle_path(
        self, leaf_hash: str, merkle_path: List[Tuple[str, str]], root: str
    ) -> bool:
        """
        ...
        for sibling_hash, position in merkle_path:
            ...
            if position == "left":
                current_hash = sha256_hash(
                    bytes.fromhex(sibling_hash) + bytes.fromhex(current_hash)
                )
            else:  # position == "right"
                current_hash = sha256_hash(
                    bytes.fromhex(current_hash) + bytes.fromhex(sibling_hash)
                )
    ...
    def append_anchor(self, anchor: AnchorRecord) -> None:
        ...
        if anchor.root != self.get_root():
            raise ValueError(
                f"Anchor root {anchor.root} does not match current tree root {self.get_root()}"
            )
...
class CapsuleBuilder:
    ...
    def build(
        ...
        """
        from .policy_enforcement import RiskLevel
        ...
        if verify_signature and public_key_pem:
            try:
                ...
                verification_results["signature_valid"] = verifier.verify(
                    anchor.get_anchor_bytes(), anchor.signature
                )
...
def make_anchor(
    root: str, policy: Policy, signer: Signer, external_anchor: Optional[str] = None
) -> AnchorRecord:
    """
