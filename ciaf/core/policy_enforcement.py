...
class PolicyRule:
    ...
    def evaluate(
        self, metadata: Dict[str, Any], policy: Policy
...
class RequiredFieldsRule(PolicyRule):
    ...
    def __init__(self):
        super().__init__(
            "REQUIRED_FIELDS", "Validates presence of required fields", RiskLevel.HIGH
        )
    ...
    def evaluate(
        ...
        if missing:
            return PolicyViolation(
                ...
                metadata={"missing_fields": missing, "record_type": record_type_str},
                timestamp=datetime.now(timezone.utc).isoformat(),
            )
...
class PolicyEnforcer:
    ...
    def _generate_recommendations(
        self, violations: List[PolicyViolation], policy: Policy
    ) -> List[str]:
        ...
        if "HIGH_RISK_DOMAIN" in violation_types:
            recommendations.append(
                "Consider additional monitoring and approval processes for high-risk domains"
            )
            recommendations.append("Implement enhanced logging and audit trails")

        if "PII_DETECTION" in violation_types:
            recommendations.append(
                "Review and potentially redact or encrypt detected PII"
            )
            recommendations.append(
                "Ensure GDPR/privacy compliance measures are in place"
            )
        ...
        if "REQUIRED_FIELDS" in violation_types:
            recommendations.append(
                "Implement input validation to ensure required fields"
            )
            recommendations.append("Review data collection processes")

        if policy.external_timestamping:
            recommendations.append(
                "Ensure external timestamping service is operational"
            )
