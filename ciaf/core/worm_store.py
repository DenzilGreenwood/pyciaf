...
class SQLiteWORMStore(WORMStore):
    ...
    def append_record(self, record: WORMRecord) -> str:
        ...
        cursor = self.conn.execute(
            "SELECT id FROM worm_records WHERE id = ?", (record.id,)
        )
        if cursor.fetchone():
...
class LMDBWORMStore(WORMStore):
    ...
    def __init__(
        self, db_path: Optional[str] = None, map_size: int = 1024 * 1024 * 1024
    ):
        """
            ...
            if record_type:
                ...
                if cursor.set_range(prefix):
                    for key, value in cursor:
                        ...
                        record_data = txn.get(
                            record_id.encode("utf-8"), db=self.records_db
                        )
                        if record_data:
...
def create_lmdb_worm_store(
    db_path: Optional[str] = None, map_size: int = 1024 * 1024 * 1024
) -> LMDBWORMStore:
