...
class LCMTrainingSession:
    ...
    def add_checkpoint(self, checkpoint: TrainingCheckpoint) -> None:
        ...
        print(
            f"   ✅ Checkpoint {checkpoint.checkpoint_id} added (epoch {checkpoint.epoch})"
        )
    ...
    def set_metrics(self, metrics: TrainingMetrics) -> None:
        ...
        print(
            f"   📊 Training metrics set (digest: {metrics.compute_metrics_digest()[:16]}...)"
        )
        ...
        enhanced_params.update(
            {
                ...
                "data_splits": {
                    split.value: anchor_id
                    for split, anchor_id in self.data_splits.items()
                },
                "checkpoints_count": len(self.checkpoints),
                "metrics_digest": (
                    self.metrics.compute_metrics_digest() if self.metrics else "none"
                ),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "data_splits": {
                split.value: anchor_id for split, anchor_id in self.data_splits.items()
            },
            "checkpoints": [cp.to_dict() for cp in self.checkpoints],
            "metrics": self.metrics.to_dict() if self.metrics else None,
            "start_time": self.start_time,
            "end_time": self.end_time,
            "training_snapshot_anchor": (
                self.get_training_snapshot_anchor() if self.training_snapshot else None
            ),
            "training_session_root": (
                self.get_training_session_root() if self.training_snapshot else None
            ),
        }
...
class LCMTrainingManager:
    ...
    def run_training_with_checkpoints(
        ...
        for epoch in range(1, epochs + 1):
            ...
            train_acc = base_train_acc + (0.85 - base_train_acc) * (
                1 - np.exp(-3.0 * progress)
            )
            val_acc = base_val_acc + (0.80 - base_val_acc) * (
                1 - np.exp(-2.8 * progress)
            )
            ...
            for cp_idx in range(checkpoints_per_epoch):
                ...
                step = (epoch - 1) * batches_per_epoch + cp_idx * (
                    batches_per_epoch // checkpoints_per_epoch
                )
                ...
                checkpoint = TrainingCheckpoint(
                    ...
                    metrics={
                        ...
                        "val_acc": val_acc,
                        "learning_rate": lr
                        * (0.95 ** (epoch - 1)),  # Learning rate decay
                        "batch_size": batch_size,
                    },
                    model_state_digest=sha256_hash(
                        f"model_weights_epoch_{epoch}_step_{step}_{hash(session.session_id)}".encode()
                    ),
                    optimizer_state_digest=sha256_hash(
                        f"optimizer_state_epoch_{epoch}_step_{step}_{hash(session.session_id)}".encode()
                    ),
                )
        ...
        print(
            f"✅ Training completed: {len(session.checkpoints)} checkpoints, final val_acc: {val_accuracies[-1]:.4f}"
        )
        print(f"   📉 Loss reduction: {train_losses[0]:.4f} → {train_losses[-1]:.4f}")
        print(
            f"   📈 Accuracy improvement: {train_accuracies[0]:.4f} → {train_accuracies[-1]:.4f}"
        )
