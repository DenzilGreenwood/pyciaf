...
class LCMRootManager:
    ...
    def compute_training_session_root(
        self, training_session: LCMTrainingSession
    ) -> str:
        """
    ...
    def compute_inference_batch_root(
        self, window_id: str, receipt_digests: List[str]
    ) -> str:
        """
    ...
    def get_test_evaluation_anchor(
        self, test_id: str
    ) -> Optional[TestEvaluationAnchor]:
    ...
    def run_test_evaluation(
        ...
        precision = np.clip(
            precision, max(0.0, accuracy - 0.1), min(1.0, accuracy + 0.1)
        )
        ...
        metrics = {
            ...
            "auc_roc": round(
                float(
                    np.clip(
                        accuracy + np.random.normal(0.05, 0.03), accuracy - 0.1, 1.0
                    )
                ),
                4,
            ),
            "log_loss": round(
                float(
                    np.clip(
                        -np.log(max(accuracy, 0.01)) + np.random.normal(0, 0.1),
                        0.01,
                        10.0,
                    )
                ),
            ...
            "confidence_mean": round(
                float(np.clip(accuracy + np.random.normal(0.1, 0.05), 0.5, 1.0)), 4
            ),
            "confidence_std": round(
                float(np.clip(np.random.gamma(2, 0.05), 0.01, 0.3)), 4
            ),
        }
        ...
        if config["compute_detailed_metrics"]:
            ...
            false_positives = int(
                total_samples * (1 - accuracy) * (1 - precision) / max(precision, 0.001)
            )
            ...
            metrics.update(
                {
                    ...
                    "specificity": round(
                        true_negatives / max(true_negatives + false_positives, 1), 4
                    ),
        ...
        print(
            f"📊 Evaluation completed: accuracy={metrics['accuracy']:.4f}, f1={metrics['f1_score']:.4f}"
        )
    ...
    def format_roots_summary(
        ...
        lines = [
            ...
            "  release_root:          R = MerkleRoot([training_session_root, predeployment_anchor,",
