...
class CIAFFramework:
    ...
    def commit_inference(self, inf_meta: Dict[str, Any]) -> Receipt:
        ...
        contains_personal_data = any(
            cat in ["personal", "sensitive", "biometric"] for cat in data_categories
        )

        if contains_personal_data:
            consent_receipts = final_meta.get("compliance_extensions", {}).get(
                "consent_receipts", []
            )
            if not consent_receipts:
                raise ComplianceError(
                    "GDPR consent receipt required for processing personal data"
                )

        # Step 7: Anchor and emit receipt
        return self._anchor_and_emit(final_meta, RecordType.INFERENCE)

    def _anchor_and_emit(
        self, metadata: Dict[str, Any], record_type: RecordType
    ) -> Receipt:
        """
    ...
    def materialize_proof_capsule(self, artifact_id: str) -> Dict[str, Any]:
        ...
        for hash_key, stored_meta in self.ledger.hash_table.items():
            if (
                stored_meta.get("dataset_id") == artifact_id
                or stored_meta.get("model_id") == artifact_id
                or stored_meta.get("inference_id") == artifact_id
            ):
    ...
    def create_model_anchor_lcm(
        ...
        model_anchor_data = {
            ...
            "lcm_tracked": True,
            "lcm_anchor_id": (
                lcm_anchor.get("anchor_id")
                if isinstance(lcm_anchor, dict)
                else str(lcm_anchor)
            ),
    ...
    def create_model_anchor(
        ...
        model_anchor_record = {
            ...
            "parameters_fingerprint": sha256_hash(
                json.dumps(model_parameters, sort_keys=True).encode()
            ),
            "architecture_fingerprint": sha256_hash(
                json.dumps(model_architecture or {}, sort_keys=True).encode()
            ),
                ...
                if dataset_id in self.dataset_anchors:
                    ...
                    model_anchor_record["authorized_dataset_anchors"][dataset_id] = {
                        "dataset_anchor": (
                            ...
                            if isinstance(dataset_anchor.dataset_anchor, bytes)
                            else dataset_anchor.dataset_anchor
                        ),
        ...
        self.audit_generators[model_name] = AuditTrailGenerator(
            model_name=model_name, compliance_frameworks=["AI_AUDIT", "TRANSPARENCY"]
        )
        ...
        print(
            f"Model anchor created with fingerprint: {model_anchor_record['parameters_fingerprint'][:16]}..."
        )
    ...
    def train_model_with_audit(
        ...
        """
        print(
            f"Starting enhanced model training with audit for: {model_name} v{model_version}"
        )

        # Verify model anchor exists
        if model_name not in self.model_anchors:
            raise ValueError(
                f"Model anchor not found for {model_name}. Create model anchor first using create_model_anchor()"
            )
        ...
        authorized_datasets = set(
            model_anchor_record["metadata"]["authorized_datasets"]
        )
        unauthorized_datasets = capsule_dataset_ids - authorized_datasets

        if unauthorized_datasets:
            raise ValueError(
                f"Unauthorized datasets detected: {unauthorized_datasets}. Model {model_name} is only authorized for: {authorized_datasets}"
            )
        ...
        enhanced_training_params.update(
            {
                ...
                "architecture_fingerprint": model_anchor_record[
        ...
        print(
            f">> Training {model_name} v{model_version} with {len(capsules)} capsules"
        )
        print(
            f"Model fingerprint: {model_anchor_record['parameters_fingerprint'][:16]}..."
        )
        ...
        print(
            f"Training completed and audit record created: {training_audit_record.event_id}"
        )
    ...
    def perform_inference_with_lcm(
        ...
        if model_name not in self.model_anchors:
            raise ValueError(
                f"Model {model_name} not found. Create model anchor first."
            )
    ...
    def perform_inference_with_audit(
        ...
        if model_name not in self.model_anchors:
            raise ValueError(
                f"Model anchor not found for {model_name}. Create model anchor first."
            )
        ...
        inference_audit_record = audit_generator.record_inference_event(
            receipt=receipt, query_metadata=query_metadata, user_id=user_id
        )

        print(
            f"Inference completed and audit record created: {inference_audit_record.event_id}"
        )
    ...
    def get_complete_audit_trail(
        self, model_name: str, include_merkle_proofs: bool = True
    ) -> Dict[str, Any]:
        """
            ...
            if dataset_id in self.dataset_anchors:
                ...
                if include_merkle_proofs and dataset_anchor.sample_hashes:
                    ...
                    dataset_audit_info[dataset_id]["sample_merkle_proof"] = {
                        ...
                        "proof": proof,
                        "verification": MerkleTree.verify_proof(
                            sample_hash, merkle_tree.get_root(), proof
                        ),
                    }
    ...
    def _verify_audit_trail_integrity(
        self, model_name: str, inference_summary: dict
    ) -> bool:
        """
    ...
    def get_performance_metrics(
        self, dataset_id: str = None, model_name: str = None
    ) -> Dict[str, Any]:
        """
        ...
        if model_name:
            if model_name in self.model_anchors:
                ...
                metrics["model"] = {
                    ...
                    "parameters_fingerprint": model_anchor_record[
                        "parameters_fingerprint"
                    ],
                    "architecture_fingerprint": model_anchor_record[
                        "architecture_fingerprint"
                    ],
                    "authorized_datasets": len(
                        model_anchor_record["metadata"]["authorized_datasets"]
                    ),
                }
                ...
                if model_name in self.inference_connections:
                    connections_summary = self.inference_connections[
                        model_name
    ...
    def lcm_complete_workflow(
        ...
        workflow_result = {
            ...
            "dataset": {"id": dataset_id, "anchor_created": True, "lcm_tracked": True},
            "model": {"name": model_name, "anchor_created": True, "lcm_tracked": True},
            "training": {
                "completed": True,
                "lcm_tracked": True,
    ...
    def register_inference_receipt(
        self, model_name: str, receipt: InferenceReceipt
    ) -> None:
        """
