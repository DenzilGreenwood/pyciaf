...
class ConsolidatedAuditAPIHandler:
    ...
    def get_audit_trail(self, entity_id: str, entity_type: str) -> Dict[str, Any]:
        ...
        if entity_type == "dataset" and self.dataset_handler:
            ...
            if model:
                ...
                if self.inference_handler:
                    inference_history = self.inference_handler.get_inference_history(
                        entity_id
                    )
                    audit_trail["trail_components"].append(
                        {
                            ...
                            "data": {
                                ...
                                "successful_inferences": len(
                                    [
                                        i
                                        for i in inference_history
                                        if i["status"] == "success"
                                    ]
                                ),
                                "failed_inferences": len(
                                    [
                                        i
                                        for i in inference_history
                                        if i["status"] == "error"
                                ...
                                "recent_inferences": inference_history[
                                    :5
                                ],  # Last 5 inferences
                            },
                        }
        ...
        self.logger.info(
            f"Audit trail generated with {len(audit_trail['trail_components'])} components"
        )
        return audit_trail
...
class ConsolidatedMetricsAPIHandler:
    ...
    def get_system_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "integrations": {
                ...
                "universal_adapter": WRAPPER_INTEGRATION_AVAILABLE
                and UNIVERSAL_ADAPTER_AVAILABLE,
            },
            "api_metrics": {
                "total_datasets": (
                    len(self.handlers.get("dataset", {}).datasets)
                    if "dataset" in self.handlers
                    else 0
                ),
                "total_models": (
                    len(self.handlers.get("model", {}).models)
                    if "model" in self.handlers
                    else 0
                ),
                "deployed_models": (
                    len(self.handlers.get("model", {}).deployed_models)
                    if "model" in self.handlers
                    else 0
                ),
                "training_sessions": (
                    len(self.handlers.get("training", {}).training_sessions)
                    if "training" in self.handlers
                    else 0
                ),
                "inference_history": (
                    len(self.handlers.get("inference", {}).inference_history)
    ...
    def get_model_metrics(self, model_name: str) -> Dict[str, Any]:
        ...
        if inference_handler:
            ...
            metrics["inference_metrics"] = {
                "total_inferences": len(inference_history),
                "successful_inferences": len(
                    [i for i in inference_history if i["status"] == "success"]
                ),
                "failed_inferences": len(
                    [i for i in inference_history if i["status"] == "error"]
                ),
                "average_duration_ms": (
                    sum(i.get("duration_ms", 0) for i in inference_history)
                    / len(inference_history)
    ...
    def get_performance_metrics(
        ...
        metrics = {"timestamp": datetime.now().isoformat(), "performance_summary": {}}
        ...
        if inference_handler:
            ...
            if filters:
                if "model_name" in filters:
                    all_inferences = [
                        i
                        for i in all_inferences
                        if i["model_name"] == filters["model_name"]
                    ]
                if "after" in filters:
                    ...
                    all_inferences = [
                        i
                        for i in all_inferences
                        if datetime.fromisoformat(i["timestamp"]) > after_time
                    ]
            ...
            if all_inferences:
                durations = [
                    i.get("duration_ms", 0)
                    for i in all_inferences
                    if i.get("duration_ms")
                ]

                metrics["performance_summary"] = {
                    "total_requests": len(all_inferences),
                    "successful_requests": len(
                        [i for i in all_inferences if i["status"] == "success"]
                    ),
                    "failed_requests": len(
                        [i for i in all_inferences if i["status"] == "error"]
                    ),
                    "average_duration_ms": (
...
class ConsolidatedCIAFAPIFramework:
    ...
    def __init__(self, policy: APIPolicy = None):
        ...
        self.logger.info(
            f"CIAF API Framework initialized in {self.policy.api_mode.value} mode"
        )
    ...
    def initialize_handlers(self, policy: APIPolicy) -> None:
        ...
        self.audit_handler.set_handlers(
            self.dataset_handler, self.model_handler, self.inference_handler
        )
    ...
    def process_api_request(
        self, endpoint: str, method: str, data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for middleware in reversed(self.middleware):
                error_response = middleware.handle_error(
                    e, {"endpoint": endpoint, "method": method}
                )
    ...
    def _route_request(self, endpoint: str, method: str, data: Dict[str, Any]) -> Any:
        ...
        if endpoint.startswith("/api/v1/datasets"):
            if method == "POST" and endpoint == "/api/v1/datasets":
                ...
                dataset_kwargs = {
                    k: v for k, v in data.items() if k not in ["dataset_id", "metadata"]
                }
                return self.dataset_handler.create_dataset(
                    data["dataset_id"], data.get("metadata", {}), **dataset_kwargs
                )
                ...
                dataset_id = endpoint.split("/datasets/")[1]
                return self.dataset_handler.update_dataset(
                    dataset_id, data.get("updates", {})
                )
        ...
        # Model endpoints
            ...
            if method == "POST" and endpoint == "/api/v1/models":
                # Extract known parameters to avoid duplication
                model_kwargs = {
                    k: v for k, v in data.items() if k not in ["model_name", "config"]
                }
                return self.model_handler.create_model(
                    data["model_name"], data.get("config", {}), **model_kwargs
                )
            elif method == "GET" and endpoint == "/api/v1/models":
                return self.model_handler.list_models(data.get("filters"))
            elif (
                method == "GET"
                and "/models/" in endpoint
                and not endpoint.endswith("/predict")
            ):
                ...
                return self.model_handler.deploy_model(
                    model_name, data.get("deployment_config", {})
                )
            elif method == "POST" and endpoint.endswith("/predict"):
                model_name = endpoint.split("/models/")[1].split("/predict")[0]
                # Extract known parameters to avoid duplication
                inference_kwargs = {
                    k: v for k, v in data.items() if k not in ["input_data"]
                }
                return self.inference_handler.perform_inference(
                    model_name, data.get("input_data"), **inference_kwargs
                )
        ...
        # Audit endpoints
            ...
            if "/trail/" in endpoint:
                ...
                return self.audit_handler.generate_compliance_report(
                    data.get("filters", {})
                )
    ...
    async def process_async_request(
        ...
        return await loop.run_in_executor(
            None, self.process_api_request, endpoint, method, data
        )
