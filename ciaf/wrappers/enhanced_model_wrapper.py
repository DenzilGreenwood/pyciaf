...
def get_components():
    ...
    return (
        _ciaf_model_wrapper,
        _adaptive_lcm_wrapper,
        _deferred_lcm_processor,
        _lcm_mode,
        _inference_priority,
        _adaptive_lcm_config,
    )
...
class EnhancedCIAFModelWrapper(CIAFModelWrapper):
    ...
    def train(
        ...
        **kwargs,
        ...
        print(
            f"[TRAIN] [{self.model_name}] Starting verifiable training for version '{version}'..."
        )
        ...
        return (
            ...
            if hasattr(training_snapshot, "snapshot_id")
            else str(training_snapshot)
        )
            ...
            training_data.append(
                {
                    "content": (
                        X_train[i].tolist()
                        if isinstance(X_train[i], np.ndarray)
                        else X_train[i]
                    ),
                    "metadata": {
                        ...
                        "target": (
                            y_train[i].item()
                            if isinstance(y_train[i], np.ndarray)
                            else y_train[i]
                        ),
                        "sample_id": i,
    ...
    def _setup_adaptive_lcm(self):
        ...
        print("[LCM] Adaptive LCM wrapper configured")
    ...
    def predict(
        ...
        **kwargs,
        ...
        return self._adaptive_predict(
            X, priority, enable_fast_mode, return_enhanced_info, **kwargs
        )
    ...
    def _adaptive_predict(
        self, X, priority, enable_fast_mode, return_enhanced_info, **kwargs
    ):
        ...
        if self.enable_metadata_tags:
            try:
                tag = create_classification_tag(
                    ...
                    confidence=enhanced_result.get("uncertainty", {}).get(
                        "confidence", 0.75
                    ),
    ...
    def predict_batch(
        ...
        if show_progress:
            ...
            print(
                f"[SUCCESS] Batch complete: {batch_time:.3f}s total, {avg_time:.4f}s avg per prediction"
            )
...
def create_enhanced_ciaf_wrapper(
    model: Any, model_name: str, fast_inference: bool = True, **kwargs
    ...
    if "default_lcm_mode" not in kwargs:
        kwargs["default_lcm_mode"] = (
            LCMMode.DEFERRED if fast_inference else LCMMode.IMMEDIATE
        )

    return EnhancedCIAFModelWrapper(model=model, model_name=model_name, **kwargs)
