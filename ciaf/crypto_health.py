...
class CryptoHealthChecker:
        ...
        for _ in range(num_tests):
            try:
                ...
                if nonce in test_nonces:
                    self.issues.append(
                        f"Nonce collision detected in {num_tests} samples"
                    )
                    return False
    ...
    def perform_health_check(self) -> CryptoHealthStatus:
        ...
        if min_salt_length < 128:  # Less than 128 bits
            self.issues.append(
                f"Salt length ({min_salt_length} bits) below minimum (128 bits)"
            )
        ...
        if not aes_gcm_ok:
            self.recommendations.append(
                "Install cryptography library for AES-GCM support"
            )
        ...
        if len(self.issues) == 0:
            ...
            "not available" in issue or "failed" in issue for issue in self.issues
        ):
