...
from .deferred_lcm import DeferredLCMProcessor, LightweightReceipt, ReceiptHasher
...
class AdaptiveLCMWrapper:
    ...
    def __init__(
        ...
        )  # Add missing current_mode attribute
    ...
    def predict(
        ...
        **kwargs,
        ...
        prediction = (
            self.base_model.predict([input_data])[0]
        ...
        if include_receipts:
            ...
            receipt_info = self._process_lcm(
                input_data, prediction, lcm_mode, result["request_id"]
            )
            result.update(receipt_info)

        return result

    def _determine_lcm_mode(
        self, priority: InferencePriority, inference_time: float
    ) -> LCMMode:
        ...
        should_defer = (
            ...
            inference_time * 1000 < self.config.immediate_threshold_ms
            and
            # System not overloaded
            system_load.get("cpu_percent", 0) < self.config.cpu_threshold_percent
            and
            # Queue not too full
    ...
    def _process_lcm(
        self, input_data: Any, prediction: Any, mode: LCMMode, request_id: str
    ) -> Dict[str, Any]:
    ...
    def _immediate_lcm(
        ...
        receipt_content = (
            f"{receipt_id}{datetime.now().isoformat()}{input_hash}{output_hash}"
        )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if self.stats["total_predictions"] > 0:
            ...
            stats["avg_lcm_time"] = (
                self.stats["total_lcm_time"] / self.stats["total_predictions"]
            )
            stats["immediate_percentage"] = (
                self.stats["immediate_lcm_count"] / self.stats["total_predictions"]
            ) * 100
            stats["deferred_percentage"] = (
                self.stats["deferred_lcm_count"] / self.stats["total_predictions"]
            ) * 100
...
if __name__ == "__main__":
    ...
    for i in range(20):
        ...
        print(
            f"Prediction {i}: Mode={result.get('lcm_mode', 'none')}, Time={result['inference_time']:.4f}s"
        )
