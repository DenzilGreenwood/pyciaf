...
def process_schema_file(file_path: Path, version: str, dry_run: bool = False) -> bool:
    ...
    try:
        ...
        # Check if version already exists
        if "version" in schema:
            print(f"[OK] {file_path.relative_to(SCHEMAS_DIR)}: Version exists ({schema['version']})")
            return False
        ...
        if not dry_run:
            ...
            print(f"[MODIFIED] {file_path.relative_to(SCHEMAS_DIR)}: Added version {version}")
        else:
            print(f"[DRY-RUN] {file_path.relative_to(SCHEMAS_DIR)}: Would add version {version}")
