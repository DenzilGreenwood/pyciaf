...
class CIAFVerifier:
    ...
    @staticmethod
    def verify_parameter_fingerprint(
        parameters: Dict[str, Any], expected_fingerprint: str
    ) -> bool:
        """
    ...
    def verify_receipt(receipt_data: Dict[str, Any]) -> bool:
        ...
        if "dataset" in receipt_data:
            ...
            if leaves and expected_root:
                calculated_root = CIAFVerifier._calculate_merkle_root_from_leaves(
                    leaves
                )
                ...
                print(
                    f"   [DATASET] Dataset ID: {dataset_info.get('dataset_id', 'N/A')}"
                )
        ...
        if "model" in receipt_data:
            ...
            if params and param_fingerprint:
                ...
                print(
                    f"[MODEL] Model parameters: {'[SUCCESS] Valid' if param_valid else '[FAIL] Invalid'}"
                )
                ...
                print(
                    f"   [CALCULATED] Calculated fingerprint: {calculated_param_fingerprint}"
                )
                if not param_valid:
                    print("   [FAIL] Parameter fingerprint mismatch!")
            ...
            if arch and arch_fingerprint:
                ...
                print(
                    f"[ARCHITECTURE] Model architecture: {'[SUCCESS] Valid' if arch_valid else '[FAIL] Invalid'}"
                )
                print(f"   [ARCH] Architecture: {arch}")
                print(f"   [EXPECTED] Expected fingerprint: {arch_fingerprint}")
                print(
                    f"   [CALCULATED] Calculated fingerprint: {calculated_arch_fingerprint}"
                )
                if not arch_valid:
                    print("   [FAIL] Architecture fingerprint mismatch!")
        ...
        if "audit_connections" in receipt_data:
            ...
            if audit_records:
                audit_valid = True
                print(
                    f"[AUDIT] Audit connections: {'[SUCCESS] Valid' if CIAFVerifier.verify_audit_connections(audit_records) else '[FAIL] Invalid'}"
                )
                ...
                for i, record in enumerate(audit_records):
                    ...
                    print(
                        f"   [EVENT] Event {i+1}: {event_type} ({'[SUCCESS]' if record_valid else '[FAIL]'})"
                    )
        ...
        print("=" * 40)
        print(
            f"[OVERALL] Overall Receipt: {'[SUCCESS] VALID' if valid else '[FAIL] INVALID'}"
        )
...
def main():
    ...
    parser.add_argument(
        "--create-sample", action="store_true", help="Create sample receipt"
    )
    parser.add_argument("--verify-merkle", help="Verify Merkle root from data file")
    parser.add_argument(
        "--verify-audit-connections", help="Verify audit connections from file"
    )
    ...
    if args.verify_merkle:
        try:
            ...
            print(
                f"[TREE] Merkle verification: {'[SUCCESS] Valid' if result else '[FAIL] Invalid'}"
            )
    ...
    if args.verify_audit_connections:
        try:
            ...
            print(
                f"[AUDIT] Audit connections verification: {'[SUCCESS] Valid' if result else '[FAIL] Invalid'}"
            )
