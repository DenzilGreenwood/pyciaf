...
def extract_ciaf_receipt_from_deferred_batch(batch_file: str):
    ...
    param_fingerprint = sha256_hash(
        json.dumps(model_params, sort_keys=True, separators=(",", ":"))
    )
    arch_fingerprint = sha256_hash(
        json.dumps(model_arch, sort_keys=True, separators=(",", ":"))
    )
...
def main():
    ...
    batch_file = (
        "../deferred_lcm_storage/audit_trails/audit_batch_20250919_104932_7805759d.json"
    )
