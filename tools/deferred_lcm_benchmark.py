...
try:
    ...
    from ciaf.wrappers.enhanced_model_wrapper import (
        EnhancedCIAFModelWrapper,
        create_enhanced_ciaf_wrapper,
    )
    ...
    from ciaf.vault.metadata_config import (
...
class PerformanceBenchmark:
    ...
    def generate_test_data(self, n_samples: int = 1000, n_features: int = 20):
        ...
        print(
            f"[DATA] Generating test dataset: {n_samples} samples, {n_features} features"
        )
        ...
        print(
            f"[SUCCESS] Dataset ready: {self.test_data['X_train'].shape[0]} training, {self.test_data['X_test'].shape[0]} test samples"
        )
    ...
    def benchmark_single_predictions(self, n_predictions: int = 100):
        ...
        print(
            f"[BENCHMARK] Benchmarking single predictions ({n_predictions} samples)..."
        )
        ...
        for name, model in self.models.items():
            ...
            for i, sample in enumerate(test_samples):
                ...
                if hasattr(model, "predict") and hasattr(model, "train"):
                    ...
                    prediction = (
                        result.get("prediction") if isinstance(result, dict) else result
                    )
                else:
                    # Standard model
            ...
            print(
                f"    [SUCCESS] {name}: {np.mean(times):.4f}s avg, {sum(times):.3f}s total"
            )
    ...
    def benchmark_batch_predictions(self, batch_size: int = 50):
        ...
        for name, model in self.models.items():
            ...
            print(
                f"    [SUCCESS] {name}: {batch_time:.3f}s total, {avg_per_sample:.4f}s avg, {len(test_samples)/batch_time:.1f} samples/sec"
            )
        ...
        for priority in priorities:
            ...
            for _ in range(10):  # Test each priority 10 times
                ...
                result = adaptive_model.predict(
                    test_sample, priority=priority, return_enhanced_info=True
                )
                inference_time = time.time() - start_time
            ...
            print(
                f"    {priority.value}: {np.mean(times):.4f}s avg, modes: {list(set(modes))}"
            )
        ...
        for name, model in self.models.items():
            if hasattr(model, "get_performance_stats"):
                ...
                if stats.get("total_predictions", 0) > 0:
                    deferred_pct = (
                        stats.get("deferred_predictions", 0)
    ...
    def calculate_improvements(self):
        ...
        if baseline not in self.results or not any(
            comp in self.results for comp in comparisons
        ):
        ...
        if f"{baseline}_single_predictions" in self.results:
            baseline_time = self.results[f"{baseline}_single_predictions"][
                "average_time"
            ]

            for comp in comparisons:
                if f"{comp}_single_predictions" in self.results:
                    comp_time = self.results[f"{comp}_single_predictions"][
                        "average_time"
                    ]
        ...
        if f"{baseline}_batch_predictions" in self.results:
            baseline_throughput = self.results[f"{baseline}_batch_predictions"][
                "throughput"
            ]

            for comp in comparisons:
                if f"{comp}_batch_predictions" in self.results:
                    comp_throughput = self.results[f"{comp}_batch_predictions"][
                        "throughput"
                    ]
                    improvement = (
                        (comp_throughput - baseline_throughput) / baseline_throughput
                    ) * 100
    ...
    def print_summary(self):
        ...
        if "priority_handling" in self.results:
            ...
            for priority, stats in self.results["priority_handling"].items():
                print(
                    f"  {priority}: {stats['average_time']:.4f}s avg, modes: {stats['modes_used']}"
                )
